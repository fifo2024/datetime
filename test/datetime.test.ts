...
describe('My datetime library', () => {
    ...
    it('测试DateTime.new()是否正确', () => {
        ...
        expect(new DateTime().toString('yyyy-M-d')).to.equal(
    ...
    it('测试DateTime.toString("yyyy-MM-dd hh:mm")是否正确', () => {
        expect(new DateTime(2020, 2, 2, 2, 2, 2).toString('yyyy-MM-dd hh:mm')).to.equal(
            '2020-02-02 02:02',
        );
    });

    it('测试DateTime.toString("yyyy-MM-dd hh:mm:ss")是否正确', () => {
        expect(new DateTime(2020, 2, 2, 2, 2, 2).toString('yyyy-MM-dd hh:mm:ss')).to.equal(
            '2020-02-02 02:02:02',
        );
    });

    it('测试DateTime.toString("yyyy-MM-dd hh:mm:ss.SSS")是否正确', () => {
        expect(new DateTime(2020, 2, 2, 2, 2, 2, 200).toString('yyyy-MM-dd hh:mm:ss.SSS')).to.equal(
            '2020-02-02 02:02:02.200',
        );
    });
    ...
    it('测试DateTime.addYears()是否正确 -- 正常年份', () => {
        expect(new DateTime(2020, 1, 29).addYears(-2).toString('yyyy-MM-dd')).to.equal(
            '2018-01-29',
        );
    });
    ...
    it('测试DateTime.addMonths()是否正确 -- 正常天数', () => {
        expect(new DateTime(2020, 2, 29).addMonths(1).toString('yyyy-MM-dd')).to.equal(
            '2020-03-29',
        );
    });

    it('测试DateTime.addMonths()是否正确 -- 闰年2月到平年2月份', () => {
        expect(new DateTime(2020, 2, 29).addMonths(12).toString('yyyy-MM-dd')).to.equal(
            '2021-02-28',
        );
    });

    it('测试DateTime.addMonths()是否正确 -- 31天月份到30天月份', () => {
        expect(new DateTime(2020, 8, 31).addMonths(1).toString('yyyy-MM-dd')).to.equal(
            '2020-09-30',
        );
    });
    ...
    it('测试DateTime.addHours()是否正确 -- 正常', () => {
        expect(
            new DateTime(2020, 12, 30, 12, 12, 12).addHours(2).toString('yyyy-MM-dd hh:mm:ss'),
        ).to.equal('2020-12-30 14:12:12');
    });

    it('测试DateTime.addHours()是否正确 -- 跨天', () => {
        expect(new DateTime(2020, 12, 30).addHours(25).toString('yyyy-MM-dd hh:mm:ss')).to.equal(
            '2020-12-31 01:00:00',
        );
    });
    ...
    it('测试DateTime.format() 是否正确 -- 2020-06-08 22:30', () => {
        expect(DateTime.format(new DateTime(2020, 6, 8, 22, 30), 'yyyy-MM-dd hh:mm')).to.equal(
            '2020-06-08 22:30',
        );
    });
    ...
    it('测试DateTime.format() 是否正确 -- 2020-06-08 22:30:30', () => {
        expect(
            DateTime.format(new DateTime(2020, 6, 8, 22, 30, 30), 'yyyy-MM-dd hh:mm:ss'),
        ).to.equal('2020-06-08 22:30:30');
    });

    it('测试DateTime.format() 是否正确 -- 2020-06-08 22:30:30.404', () => {
        expect(
            DateTime.format(new DateTime(2020, 6, 8, 22, 30, 30, 404), 'yyyy-MM-dd hh:mm:ss.SSS'),
        ).to.equal('2020-06-08 22:30:30.404');
    });
    ...
    it('测试DateTime.equalTo() 是否正确 -- true', () => {
        expect(new DateTime(2021, 1, 9).equalTo(new DateTime(2021, 1, 8).addDays(1))).to.equal(
            true,
        );
    });
    ...
    it('测试DateTime.today() 是否正确 -- 2021-02-09', () => {
        const current = new Date();
        expect(DateTime.today().toString('yyyy-M-d')).to.equal(
            `${current.getFullYear()}-${current.getMonth() + 1}-${current.getDate()}`,
        );
    });
});
